...
return view.extend({
	...
	render: function(data) {
		...
		const log = data[1] || {};
		const sites = data[2] || {};
		const peersData = data[4] || { peers: [], whitelist: [] };
		const formNode = data[3];
		...
		const saveSitesBtn = E('button', {
			...
			click: function() {
				...
				set_sites({ content: sitesTextarea.value || '' }).then(function(r) {
					...
					ui.addNotification(r && r.ok ? null : (r && r.error) || _('Failed'), r && r.ok ? _('Sites saved.') : (r && r.error) || _('Failed'), r && r.ok ? 'info' : 'error');
				}).catch(function() { saveSitesBtn.disabled = false; });
			}
		}, _('Save site list'));
		...
		const savePeersBtn = E('button', { class: 'btn cbi-button cbi-button-apply', click: function() {
		...
		const refreshPeersBtn = E('button', { class: 'btn cbi-button', click: function() {
		...
		const endpointsSection = E('div', { class: 'cbi-section' }, [
			E('h3', { class: 'cbi-section-title' }, _('VPN endpoints (polling whitelist)')),
